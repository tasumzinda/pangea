...
public class ARTFormAdapter extends ArrayAdapter<ARTForm>  {
    ...
    private ArrayList<ARTForm> list;

    public ARTFormAdapter(Context context, ArrayList<ARTForm> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ARTForm item = list.get(position);
        holder.name.setText((position + 1) + ". " + item.period + " " + item.facility);
        return rowView;
    }

    public void changeDataSet(ArrayList<ARTForm> arrayList) {
        list.clear();
